...
//   v === 'morning' ? 'MORNING' : v === 'evening' ? 'EVENING' : 'NONE';
...
//       console.log('Sending request to /recruit...');
//       console.log('FormData keys:', Object.keys(form));
      
//       const res = await api.post('/recruit', form, {
//         headers: {
...
/** 프론트 값 → 서버 enum 문자열 매핑 */
const toLifestyleEnum = (v: string): Lifestyle => {
  switch (v) {
    case 'morning': return Lifestyle.Morning;
    case 'evening': return Lifestyle.Evening;
    default: return Lifestyle.Morning; // 기본값
  }
};

const toPersonalityEnum = (v: string): Personality => {
  switch (v) {
    case 'introvert': return Personality.Introvert;
    case 'extrovert': return Personality.Extrovert;
    default: return Personality.Introvert; // 기본값
  }
};

const toSmokingEnum = (v: string): boolean => {
  switch (v) {
    case 'impossible': return false;  // 흡연 불가
    case 'none': return true;          // 상관없음
    default: return false;
  }
};

const toSnoringEnum = (v: string): boolean => {
  switch (v) {
    case 'impossible': return false;  // 코골이 불가
    case 'none': return true;          // 상관없음
    default: return false;
  }
};
...
const handleAddressSelect = (addr: AddressResult) => {
  ...
  /** 제출 → /recruit (POST) - 이미지 없이 JSON만 전송 */
  const handleSubmit = async () => {
    console.log('handleSubmit called');
    try {
      console.log('Creating requestDto...');
      ...
      // ===== 수정: 타입 안전하게 변환 =====
      const requestDto: RecruitRequest = {
        title: formData.title,
        recruitCount: Number(formData.recruitCount),
        rentCostMin: Number(formData.depositMin),
        rentCostMax: Number(formData.depositMax),
      ...
      console.log('RequestDto created:', JSON.stringify(requestDto, null, 2));

      // ===== 수정: JSON으로 직접 전송 (multipart 제거) =====
      console.log('Sending request to /recruit...');
      
      const res = await api.post('/recruit', requestDto, {
        headers: {
      ...
      // 상세한 에러 메시지 표시
      const errorMessage = e?.response?.data?.message || e?.message || '등록 중 오류가 발생했습니다.';
      Alert.alert('실패', `${errorMessage}\n\n상태 코드: ${e?.response?.status || 'N/A'}`);
    }
  };
  ...
  const renderStep4 = () => (
    <View style={styles.stepContainer}>
      <View style={styles.stepHeader}>
        <Text style={styles.stepTitle}>주소 정보</Text>
        <Text style={styles.stepSubtitle}>
          {formData.hasRoom === 'has' ? '방 주소를 검색해 주세요' : '희망 지역을 입력해 주세요'}
        </Text>
      </View>
